...
'           SQL = SQL + " FROM (((BookPOParent P INNER JOIN BookPOChild05 C ON P.Code=C.Code) INNER JOIN BookMaster I ON P.Book=I.Code) INNER JOIN ElementMaster E ON C.Element=E.Code) INNER JOIN GeneralMaster H ON I.HSNCode=H.Code WHERE P.Code+E.Code+'XXXXXXMF' IN (" & FrmOrderList.VchCodeList & ")"
            ...
            SQL = SQL + " UNION ALL "
            SQL = SQL + "SELECT P.Code AS oCode,I.Name+'_Binding' As ItemName,I.Code As ItemCode,H.Code As HSNCode,H.Name As HSNName,ROUND((C.BillAmount-C.VAT)/C.ActualQuantity,3) As UnitRate,P.ProfitMargin,(SELECT Code+'-'+Name FROM GeneralMaster WHERE Type='17' AND Value1=1) As Narration,LTRIM(P.Name)+'/'+" & IIf(InStr(1, "Q_Z", Left(VchType, 1)) > 0, "LEFT(P.Type,1)", "RIGHT(P.Type,1)") & "+'O/BN' As VchNo,P.Code+'XXXXXXXXXXXXBN' As VchCode,"
            If cmbBillingType.ListIndex = 0 Then 'Direct
...
Public Sub PrintJobworkBillVch(ByVal VchCode As String, ByVal VchType As String, ByVal BillType As String, Optional ByVal OutputType As String)
    ...
    ElseIf InStr(1, "SJ_PJ_QJ_ZJ", Right(VchType, 2)) > 0 Then
                                                            ...
                                                            SQL = SQL + "'' As cmbTitle," & FS01 & " As FinishSize,LongNarration01,LongNarration02,LongNarration03,LongNarration04,LongNarration05,"
                                                            SQL = SQL + "'Text Plates: '+ "
                                                            SQL = SQL + "IIF(((C2.[TotalPlates1-¼]+C2.[TotalPlates1-½]+C2.[TotalPlates1-1]+C2.[RevisedPlates1])*1)=0,'',LTRIM((C2.[TotalPlates1-¼]+C2.[TotalPlates1-½]+C2.[TotalPlates1-1]+C2.[RevisedPlates1])*1)+ '-plates- 1 Col, ')+"
                                                            SQL = SQL + "IIF(((C2.[TotalPlates2-¼]+C2.[TotalPlates2-½]+C2.[TotalPlates2-1]+C2.[RevisedPlates2])*2)=0,'',LTRIM((C2.[TotalPlates2-¼]+C2.[TotalPlates2-½]+C2.[TotalPlates2-1]+C2.[RevisedPlates2])*2)+ '-plates- 2 Col, ')+"
                                                            SQL = SQL + "IIF(((C2.[TotalPlates4-¼]+C2.[TotalPlates4-½]+C2.[TotalPlates4-1]+C2.[RevisedPlates4])*4)=0,'',LTRIM((C2.[TotalPlates4-¼]+C2.[TotalPlates4-½]+C2.[TotalPlates4-1]+C2.[RevisedPlates4])*4)+ ' - plates- 4 Col')+'  = ' +LTRIM(((C2.[TotalPlates1-¼]+C2.[TotalPlates1-½]+C2.[TotalPlates1-1]+C2.[RevisedPlates1])*1)+((C2.[TotalPlates2-¼]+C2.[TotalPlates2-½]+C2.[TotalPlates2-1]+C2.[RevisedPlates2])*2)+((C2.[TotalPlates4-¼]+C2.[TotalPlates4-½]+C2.[TotalPlates4-1]+C2.[RevisedPlates4])*4))+'  Nos.' AS Plates,"
                                                            ...
                                                            SQL = SQL + "(Select Name From ElementMaster Where Code=SubString(BOM,5,6) And Right(C1.BOM,2)='ME')+': '+LTRIM(C3.TotalPlates)+ '- plates  , Color: ( '+LTRIM(C3.FrontPrintingType)+' + '+LTRIM(C3.BackPrintingType)+' )' AS Plates,"
                                                            ...
                                                            'BN
                                                            SQL = SQL + oSQL + "'' As cmbTitle," & FS01 & " As FinishSize,LongNarration01,LongNarration02,LongNarration03,LongNarration04,LongNarration05,"
                                                            SQL = SQL + "(Select Name From GeneralMaster Where Code=C4.BindingType)+' : '+IIF((C4.bindingForms+C4.ExtraForms)=0,'',Convert(nvarchar,(C4.bindingForms+C4.ExtraForms))+ ' Forms ') AS Plates,"
                                                            SQL = SQL + "'Amount:=' + IIF(C4.FormFoldRate=0,'',' ( Folding: '+Convert(nvarchar,(C4.BillingQuantity*(C4.bindingForms+C4.ExtraForms)))+' X '+Convert(nvarchar,C4.FormFoldRate)+'@ = Rs.'+Convert(nvarchar,Format(C4.BillingQuantity*(C4.bindingForms+C4.ExtraForms)*C4.FormFoldRate/1000,'#####00'))+' ) + ')"
                                                            SQL = SQL + "+ IIF(C4.FormStitchRate=0,'','( Stitching: '+Convert(nvarchar,(C4.BillingQuantity*(C4.bindingForms+C4.ExtraForms)))+' X '+Convert(nvarchar,C4.FormStitchRate)+'@ = Rs.'+Convert(nvarchar,Format(C4.BillingQuantity*(C4.bindingForms+C4.ExtraForms)*C4.FormStitchRate/1000,'#####00'))+' ) + ')"
                                                            SQL = SQL + "+ IIF(C4.FormPasteRate=0,'','( Binding: '+Convert(nvarchar,(C4.BillingQuantity))+' X '+Convert(nvarchar,C4.FormPasteRate)+'@ = Rs.'+Convert(nvarchar,Format(C4.BillingQuantity*C4.FormPasteRate/1000,'#####00'))+' ) + ')"
                                                            SQL = SQL + "+ IIF(C4.[Rate/Book]=0,'','( Unit: '+Convert(nvarchar,(C4.BillingQuantity))+' X '+Convert(nvarchar,C4.[Rate/Book])+'@ = Rs.'+Convert(nvarchar,Format(C4.BillingQuantity*C4.[Rate/Book],'#####00'))+' ) + ')"
                                                            SQL = SQL + "+ IIF(C4.PktPackRate=0,'','( Shrink Packing: '+Convert(nvarchar,(C4.TotalPkts))+' X '+Convert(nvarchar,C4.PktPackRate)+'@ = Rs.'+Convert(nvarchar,Format(C4.TotalPkts*C4.PktPackRate,'#####00'))+' ) + ')"
