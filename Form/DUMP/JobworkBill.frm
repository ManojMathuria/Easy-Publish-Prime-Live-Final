...
Private Sub PushVch()
    ...
    With rstJobworkBVChild
        If .State = adStateOpen Then .Close
        XMLStr = "SELECT Left((Select V.Prefix From VchSeriesMaster V Where Code=VchSeries),2)+'_'+'" & Right(VchType, 2) & "'+'/'+Ltrim(AutoVchNo)+'/'+ '" & FYFromTo & "'  As BillNo,H.Date As BillDate,M.PrintName As MatCentre," + _
...
Public Sub PrintJobworkBillVch(ByVal VchCode As String, ByVal VchType As String, ByVal BillType As String, Optional ByVal OutputType As String)
    ...
    FS01 = "'Size: '+LTRIM(S.PrintName)+IIf(I.Pages = 0, '', ', '+LTRIM(I.Pages)+' pages/'+LTRIM(I.Forms)+'f ('+LTRIM(IIF(OneColorForms<>0,LTRIM(OneColorForms)+'f-1Col','')+' '+IIF(TwoColorForms<>0,LTRIM(TwoColorForms)+'f-2Col','')+' '+IIF(FourColorForms<>0,LTRIM(FourColorForms)+'f-4Col',''))+')')" 'Direct Billing
    ...
    rstCompanyMaster.ActiveConnection = Nothing
      SQL = "SELECT LTRIM(P.Name) +'/' +'" & Right(Year(FinancialYearFrom), 2) + "-" + Right(Year(FinancialYearTo), 2) & "' As BillNo,P.Date As BillDate," & _
                ...
                "N.PrintName As Narration,'StockItem: '+I.PrintName As Item,H.PrintName As HSNCode,C1.Quantity,C1.Rate,C1.Amount,N.Name As SrNo,LTRIM(C1.Code)+LTRIM(C1.SrNo) As Ref,"
    cnJobworkBill.CommandTimeout = 300
    If Right(VchType, 2) = "SC" Or Right(VchType, 2) = "PC" Or Right(VchType, 2) = "QC" Or Right(VchType, 2) = "ZC" Then
            rstJobworkBVChild.Open SQL & "'' As cmbTitle," & FS01 & ",LongNarration01,LongNarration02,LongNarration03,LongNarration04,LongNarration05," & _
                "ISNULL('Paper Consumption : '+(Convert(nvarchar,Round(Format((ISNULL((Select PaperConsumptionsheets1 From BookPOChild05 Where Code=P2.Code),0)+ISNULL((Select PaperConsumptionsheets2 From BookPOChild05 Where Code=P2.Code),0)+ISNULL((Select PaperConsumptionsheets4 From BookPOChild05 Where Code=P2.Code),0)+ISNULL((Select PaperConsumptionsheets From BookPOChild06 Where Code=P2.Code),0)+ISNULL((Select PaperConsumptionsheets From BookPOChild09 Where Code=P2.Code),0))/P2.EstQty01*C1.Quantity,'00.00'),.5))+' Sheets'),'') As PaperConsumptionsheets, " & _
                "IIF(C1.Ref IS NULL,'','Ref. No. :')+(CASE WHEN C1.Ref IS NULL THEN '' ELSE LTRIM(P2.Name)+'/'+RIGHT(P2.Type,1)+'O/'+RIGHT(C1.BOM,2) END) As RefOrderNo, " & _
                ...
                "(C2.[TotalPlates1-¼]+C2.[TotalPlates1-½]+C2.[TotalPlates1-1]+C2.[RevisedPlates1])*1 As TotalPlates1,((C2.[TotalPlates1-¼]+C2.[TotalPlates1-½]+C2.[TotalPlates1-1]+C2.[RevisedPlates1])*1)*PlateRate1 As PlateAmount1,(C2.[TotalPlates2-¼]+C2.[TotalPlates2-½]+C2.[TotalPlates2-1]+C2.[RevisedPlates2])*2 As TotalPlates2,((C2.[TotalPlates2-¼]+C2.[TotalPlates2-½]+C2.[TotalPlates2-1]+C2.[RevisedPlates2])*2)*PlateRate2 As PlateAmount2,(C2.[TotalPlates4-¼]+C2.[TotalPlates4-½]+C2.[TotalPlates4-1]+C2.[RevisedPlates4])*4 As TotalPlates4,((C2.[TotalPlates4-¼]+C2.[TotalPlates4-½]+C2.[TotalPlates4-1]+C2.[RevisedPlates4])*4)*PlateRate4 As PlateAmount4,Left((Select V.Prefix From VchSeriesMaster V Where Code=VchSeries),2)+'_'+'" & Right(VchType, 2) & "'+'/'+Ltrim(AutoVchNo) As VchNo " & _
    ...
    ElseIf Right(VchType, 2) = "SU" Or Right(VchType, 2) = "PU" Or Right(VchType, 2) = "QU" Or Right(VchType, 2) = "ZU" Then  '"SG"
        rstJobworkBVChild.Open SQL & "'' As cmbTitle," & FS01 & " As FinishSize,LongNarration01,LongNarration02,LongNarration03,LongNarration04,LongNarration05, " & _
                                                                                    ...
                                                                                    "IIF(C1.Ref IS NULL,'','Ref. No. :')+(CASE WHEN Ref IS NULL THEN '' ELSE LTRIM(R.Name)+'/'+RIGHT(R.Type,1)+'O/'+RIGHT(C1.BOM,2) END) As RefOrderNo,Left((Select V.Prefix From VchSeriesMaster V Where Code=VchSeries),2)+'_'+'" & Right(VchType, 2) & "'+'/'+Ltrim(AutoVchNo) As VchNo " & _
    ...
    ElseIf InStr(1, "SJ_PJ_QJ_ZJ", Right(VchType, 2)) > 0 Then
                                                            SQL = SQL + "Left((Select V.Prefix From VchSeriesMaster V Where Code=VchSeries),2)+'_'+'SJ'+'/'+Ltrim(AutoVchNo) As VchNo "
                                                            SQL = SQL + "FROM((((((JobworkBVParent P INNER JOIN JobworkBVChild C1 ON P.Code=C1.Code) LEFT JOIN BookMaster I ON C1.Item=I.Code) LEFT JOIN GeneralMaster N ON C1.Narration=N.Code) LEFT JOIN GeneralMaster H ON C1.HSNCode=H.Code) LEFT JOIN BookPOParent P2 ON C1.Ref=P2.Code) LEFT JOIN AccountMaster A ON P.Party=A.Code) LEFT JOIN AccountMaster C ON P.Consignee=C.Code "
                                                            SQL = SQL + "WHERE P.Code='" + VchCode + "'  ORDER BY Item,SrNo"
