...
Private Sub fpSpread1_LeaveCell(ByVal Col As Long, ByVal Row As Long, ByVal NewCol As Long, ByVal NewRow As Long, Cancel As Boolean)
    On Error GoTo ErrorHandler
    Dim ActiveCellVal As Variant, CalcType As String, Number  As Variant, Size As Variant, Qty As Variant, CalcMode As Variant, Rate As Variant, xPos As Integer, Amount As Double, Adjustment As Variant, GST As Variant, Operation As Variant
    With fpSpread1
        ...
        If Not CheckEmpty(CalcMode, False) Then
            ...
            If Not rstCalcModeList.EOF Then CalcType = IIf(InStr(1, rstCalcModeList.Fields("Name").Value, "Inch") > 0, "S", "O") ': CalcVal = Val(rstCalcModeList.Fields("Value1").Value)
        ...
        If Val(Rate) = 0 Then
            ...
            .GetText 16, Row, Size
            If Not (CheckEmpty(Operation, False) And CheckEmpty(CalcMode, False)) And Val(Qty) > 0 Then .SetText 9, Row, FetchOperationRate(Operation, CalcMode, IIf(CalcType = "S", Size, ""), Val(Qty))
        End If
...
Private Sub RefreshDropDownList(ByVal xType As String)
    ...
    End With
End Sub
Private Function FetchOperationRate(ByVal xOperation As String, ByVal xCalcMode As String, ByVal xSize As String, xRange As Double) As Double
    On Error GoTo ErrorHandler
    ...
    With rstOrderList
        ...
        If .RecordCount = 0 Then
            ...
            With fpSpread1
                ...
                If rstFetchOperationRate.State = adStateOpen Then rstFetchOperationRate.Close
                rstFetchOperationRate.Open "SELECT Rate FROM AccountChild07 WHERE Code = '" & PartyCode & "' AND LaminationType='" & Operation & "' AND CalcMode='" & CalcMode & "' AND [Size]='" & Size & "'", cnDatabase, adOpenKeyset, adLockReadOnly
                'Rate
