...
Dim PaperCode As String, ItemCode As String, BookPrinterCode As String, TitlePrinterCode As String, LaminatorCode As String, BinderCode As String, MaterialCentreCode As String
...
'    Set FI = CreateObject("Busy2L16.CFixedInterface")
 '   FI.OpenCSDB Trim(ReadFromFile("BusyPath")), Trim(ReadFromFile("Server Name")), "sa", Trim(ReadFromFile("Server Password")), Mid(cnBusy.DefaultDatabase, 5, 8)
    ...
    CenterForm Me
    WheelHook DataGrid1
    Me.Caption = IIf(BookPOType = "DS", "Sales Order [Digital Printing]", IIf(BookPOType = "DP", "Purchase Order [Digital Printing]", IIf(BookPOType = "FP", "Purchase Order [Finished Goods]", IIf(BookPOType = "RP", "Purchase Order [Unfinished Goods]", IIf(BookPOType = "OP", "Cost Estimation", IIf(BookPOType = "FS", "Sales Order [Finished Goods]", "Sales Order [Unfinished Goods]"))))))
...
Private Sub Text3_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeySpace Then
        ...
        FrmBookMaster.BookType = IIf(Left(BookPOType, 1) = "D", "F", IIf(Left(BookPOType, 1) = "O", "F", Left(BookPOType, 1)))
...
Private Sub ClearFields()
    ...
    Command1.Enabled = True: Command3.Enabled = True: Command5.Enabled = True
End Sub
...
ElseIf Left(BookPOType, 1) = "D" Then
    If CheckEmpty(Text5.Text, False) Then Exit Sub
    If rstBookPOChild05.RecordCount = 0 Then Call AddRecord(rstBookPOChild05)
    Set FrmBookPOChild02.rstBookPOChild05 = rstBookPOChild05
    FrmBookPOChild02.VchType = BookPOType
    FrmBookPOChild02.PrinterCode = BookPrinterCode
    If Right(BookPOType, 1) = "P" Then FrmBookPOChild02.Mh3dLabel46.Caption = " Paper Supplied" Else FrmBookPOChild02.Mh3dLabel46.Caption = " Paper by Party"
    On Error Resume Next
    ...
    If Err.Number <> 364 Then
        If rstBookPOParent.Fields("BookPrinter").Value <> "" Then
            If Left(BookPOType, 1) <> "O" Then
                If blnRecordExist And AllowTransactionsModification = 0 Then
                    If Not CheckEmpty(FrmBookPOChild02.Text8.Text, False) Then
                        'Dim O As Object
                        For Each O In FrmBookPOChild02
                                If TypeName(O) = "TextBox" Then
                                    O.Locked = True
                                ElseIf TypeName(O) = "TDBNumber" Then
                                    O.ReadOnly = True
...
'        End If
    ...
    Else
            rstBookPOChild06.Open "SELECT (SELECT PrintName FROM ElementMaster WHERE Code=C.Element) As Element,Trim(M1.PrintName)+iif(M1.Price=0,'',' (Price : Rs. '+Format(M1.Price,'0.00')+')') As Item,M2.PrintName As FinishSize,TRIM(TRIM(M1.Pages)+'pages/'+TRIM(M1.Forms)+'f('+IIF(M1.OneColorForms=0,'','1-Col_'+TRIM(M1.OneColorForms)+'f +')+IIF(M1.TwoColorForms=0,'','2-Col_'+TRIM(M1.TwoColorForms)+'f ')+IIF(M1.FourColorForms=0,'',' + 4-Col-'+TRIM(M1.FourColorForms)+'f '))+')' As Forme,TRIM(P.Name) As OrderNo,P.Date As OrderDate,(SELECT PrintName FROM AccountMaster WHERE Code=P.TitlePrinter) As CoverPrinter,C.ActualQuantity,C.BillingQuantity,C.FrontPrintingType,C.BackPrintingType,Imposition,(SELECT PrintName FROM GeneralMaster WHERE Code=C.[Size]) As [Size],CHOOSE(VAL(PlateType),'Deep-etch','PS','Wipe-on','CTP') As Plate,PrintRate,PrintAmount,PlateRate,PlateAmount,(SELECT TRIM(M3.PrintName)FROM PaperMaster M3 INNER JOIN GeneralMaster M4 ON M3.UOM=M4.Code WHERE M3.Code=C.Paper) As PaperName," & _
                                  "[PaperWastage%],PaperConsumptionOther,Adjustment,PAdjustment,[VAT%],VAT,[PVAT%],PVAT,BillAmount,PBillAmount,C.Remarks,(SELECT TRIM(eMail) FROM AccountMaster WHERE Code=P.TitlePrinter) As EMailId,M1.Narration,P.TitlePrinter,PlateMaker,PaperWastageMin,PaperRate,PaperAmount,RBillAmount,RAdjustment,[RVAT%],RVAT,C.Processing,C.ProcessingBack,C.[Titles/sheet1] As Ups,(SELECT '('+TRIM(M6.PrintName)+')' FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper) As UOM,P.EstQty01 As FinalQuantity,C.Sets,C.TotalPlates,C.TotalPlatesBack,C.Ref,CHOOSE(VAL(PlateTypeBack),'Deep-etch','PS','Wipe-on','CTP') As PlateBack,C.Pages  " & _
                                  "FROM ((BookPOParent P INNER JOIN BookPOChild06 C ON P.Code=C.Code) INNER JOIN BookMaster M1 ON P.Book=M1.Code) INNER JOIN GeneralMaster M2 ON M1.FinishSize=M2.Code WHERE P.Code='" & OrderCode & "' AND P.TitlePrinter='" & rstBookPOChild0801.Fields("Vendor").Value & "'", cnDatabase, adOpenKeyset, adLockOptimistic
    End If
...
Public Sub JobCard(ByVal OrderCode As String, Optional ByVal Note As String, Optional ByVal OutputType As String, Optional ByVal OrderType As String, Optional ByVal BookPOType As String)
    ...
    Else
            rstBookPOChild06.Open "SELECT (SELECT PrintName FROM ElementMaster WHERE Code=C.Element) As Element,LTrim(M1.PrintName)+iif(M1.Price=0,'',' (Price : Rs. '+Format(M1.Price,'0.00')+')') As Item,M2.PrintName As FinishSize,LTrim(LTrim(M1.Pages)+'pages/'+LTrim(M1.Forms)+'f ('+IIF(M1.OneColorForms=0,'','1-Col_'+LTrim(M1.OneColorForms)+'f  +')+IIF(M1.TwoColorForms=0,'',' 2-Col_'+LTrim(M1.TwoColorForms)+'f ')+IIF(M1.FourColorForms=0,'','+ 4-Col_'+LTrim(M1.FourColorForms)+'f '))+' )' As Forme,LTrim(P.Name) As OrderNo,P.Date As OrderDate,(SELECT PrintName FROM AccountMaster WHERE Code=P.TitlePrinter) As CoverPrinter,C.ActualQuantity,C.BillingQuantity,C.FrontPrintingType,C.BackPrintingType,Imposition,(SELECT PrintName FROM GeneralMaster WHERE Code=C.[Size]) As [Size],CHOOSE(VAL(PlateType),'Deep-etch','PS','Wipe-on','CTP') As Plate,PrintRate,PrintAmount,PlateRate,PlateAmount,(SELECT LTrim(M3.PrintName)FROM PaperMaster M3 INNER JOIN GeneralMaster M4 ON M3.UOM=M4.Code WHERE M3.Code=C.Paper) As PaperName," & _
                                  "[PaperWastage%],PaperConsumptionOther,Adjustment,PAdjustment,[VAT%],VAT,[PVAT%],PVAT,BillAmount,PBillAmount,C.Remarks,(SELECT LTrim(eMail) FROM AccountMaster WHERE Code=P.TitlePrinter) As EMailId,M1.Narration,P.TitlePrinter,PlateMaker,PaperWastageMin,PaperRate,PaperAmount,RBillAmount,RAdjustment,[RVAT%],RVAT,C.Processing,C.ProcessingBack,C.[Titles/sheet1] As Ups,(SELECT '('+LTrim(M6.PrintName)+')' FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper) As UOM,P.EstQty01 As FinalQuantity,C.Sets,C.TotalPlates,C.TotalPlatesBack,C.Ref,CHOOSE(VAL(PlateTypeBack),'Deep-etch','PS','Wipe-on','CTP') As PlateBack,C.Pages  " & _
                                  "FROM ((BookPOParent P INNER JOIN BookPOChild06 C ON P.Code=C.Code) INNER JOIN BookMaster M1 ON P.Book=M1.Code) INNER JOIN GeneralMaster M2 ON M1.FinishSize=M2.Code WHERE P.Code='" & OrderCode & "' AND P.TitlePrinter='" & rstBookPOChild0801.Fields("Vendor").Value & "'", cnDatabase, adOpenKeyset, adLockOptimistic
    End If
            If rstBookPOChild06.RecordCount = 0 Then
                rptJobCard.Section4.Suppress = True
...
'            rptQuotationFormat.Section14.Suppress = True
    ...
    Screen.MousePointer = vbNormal
End Sub
Public Sub PrintBookPrintOrder02(ByVal OrderCode As String, Optional ByVal Note As String, Optional ByVal OutputType As String, Optional ByVal OrderType As String, Optional ByVal BookPOType As String)
    Dim oOutlookMsg As Outlook.MailItem, HeaderPrinted As Boolean, OrderNo As String, ItemName As String, TotalTax As Double, TotalAmount As Double, BillAmount As Double, PBillAmount As Double, RBillAmount As Double, BOMAmount As Double
    Dim profitFlag As Boolean
    On Error Resume Next
    Screen.MousePointer = vbHourglass
    rptBookPrintOrder02.Text1.SetText IIf(Left(BookPOType, 1) = "D", "Digital Printing Order", IIf(Right(BookPOType, 1) = "S", "Proforma Invoice", IIf(Left(BookPOType, 1) = "O", "Quotation/Estimation", "Item Order")))
...
'        rptBookPrintOrder02.Section17.Suppress = True
    ...
    ElseIf OrderType = "BB" Then
        rstBookPOChild0801.Open "SELECT Binder As Vendor FROM BookPOParent P INNER JOIN BookPOChild08 C ON P.Code=C.Code WHERE P.Code='" & OrderCode & "'", cnDatabase, adOpenKeyset, adLockOptimistic
    Else
        rstBookPOChild0801.Open "SELECT BookPrinter As Vendor FROM BookPOParent P INNER JOIN BookPOChild05 C ON P.Code=C.Code WHERE P.Code='" & OrderCode & "' UNION SELECT TitlePrinter As Vendor FROM BookPOParent P INNER JOIN BookPOChild06 C ON P.Code=C.Code WHERE P.Code='" & OrderCode & "' UNION SELECT TitlePrinter As Vendor FROM BookPOParent P INNER JOIN BookPOChild09 C ON P.Code=C.Code WHERE P.Code='" & OrderCode & "' UNION SELECT Laminator As Vendor FROM BookPOParent P INNER JOIN BookPOChild07 C ON P.Code=C.Code WHERE P.Code='" & OrderCode & "' UNION SELECT Binder As Vendor FROM BookPOParent P INNER JOIN BookPOChild08 C ON P.Code=C.Code WHERE P.Code='" & OrderCode & "'", cnDatabase, adOpenKeyset, adLockOptimistic
    End If
    If rstBookPOChild0801.RecordCount > 0 Then rstBookPOChild0801.MoveFirst
    Do While Not rstBookPOChild0801.EOF
        TotalTax = 0: TotalAmount = 0
        ...
        If OrderType = "BP" Or OrderType = "ALL" Then
            If rstBookPOChild05.State = adStateOpen Then rstBookPOChild05.Close
    If DatabaseType = "MS SQL" Then
            rstBookPOChild05.Open "SELECT LTRIM(M1.PrintName)+iif(M1.Price=0,'',' (Price : Rs. '+Format(M1.Price,'0.00')+')') As Item,M2.PrintName As FinishSize,LTRIM(LTRIM(M1.Pages)+'-pages/'+LTRIM(M1.Forms)+'f('+IIF(M1.OneColorForms=0,'','1Col-'+LTRIM(M1.OneColorForms)+'f ')+IIF(M1.TwoColorForms=0,'',' 2Col-'+LTRIM(M1.TwoColorForms)+'f ')+IIF(M1.FourColorForms=0,'',' 4Col-'+LTRIM(M1.FourColorForms)))+IIF(M1.FourColorForms>0,'f ',' ')+')' As Forme,LTRIM(P.Name) As OrderNo,P.Date As OrderDate,(SELECT PrintName FROM AccountMaster WHERE Code=P.BookPrinter) As TextPrinter,C.ActualQuantity,M1.DuplexPrinting,BillingQuantity01,BillingQuantity02,(SELECT PrintName+'/'+CHOOSE(M1.FormType,'08','16','04','12','24','32','64','06') FROM GeneralMaster WHERE Code=C.Size1) As [Size1],Pages1,[Forms1-¼],[Forms1-½],[Forms1-1],CHOOSE(CONVERT(NUMERIC,PlateType1),'Deep-etch','PS','Wipe-on','CTP') As Plate1,PrintRate1,PrintAmount1,PlateRate1,PlateAmount1," & _
                                  "(SELECT LTRIM(M3.PrintName) FROM PaperMaster M3 INNER JOIN GeneralMaster M4 ON M3.UOM=M4.Code WHERE M3.Code=C.Paper1) As Paper1Name,[PaperWastage1%],PaperConsumptionOther1,(SELECT ''+LTrim(M6.PrintName)+' ' FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper1) As UOM1,(SELECT LTrim(M7.[Weight/Unit]) FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper1) As UOM1wt,(SELECT LTrim(M6.[Value1]) FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper1) As UOM1qty," & _
                                  "(SELECT PrintName+'/'+CHOOSE(M1.FormType,'08','16','04','12','24','32','64','06') FROM GeneralMaster WHERE Code=C.Size2) As [Size2],Pages2,[Forms2-¼],[Forms2-½],[Forms2-1],CHOOSE(CONVERT(NUMERIC,PlateType2),'Deep-etch','PS','Wipe-on','CTP') As Plate2,PrintRate2,PrintAmount2,PlateRate2,PlateAmount2," & _
                                  "(SELECT LTRIM(M3.PrintName) FROM PaperMaster M3 INNER JOIN GeneralMaster M4 ON M3.UOM=M4.Code WHERE M3.Code=C.Paper2) As Paper2Name,[PaperWastage2%],PaperConsumptionOther2,(SELECT ''+LTrim(M6.PrintName)+' ' FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper2) As UOM2,(SELECT LTrim(M7.[Weight/Unit]) FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper2) As UOM2wt,(SELECT LTrim(M6.[Value1]) FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper2) As UOM2qty," & _
                                  "(SELECT PrintName+'/'+CHOOSE(M1.FormType,'08','16','04','12','24','32','64','06') FROM GeneralMaster WHERE Code=C.Size4) As [Size4],Pages4,[Forms4-¼],[Forms4-½],[Forms4-1],CHOOSE(CONVERT(NUMERIC,PlateType4),'Deep-etch','PS','Wipe-on','CTP') As Plate4,PrintRate4,PrintAmount4,PlateRate4,PlateAmount4," & _
                                  "(SELECT LTRIM(M3.PrintName) FROM PaperMaster M3 INNER JOIN GeneralMaster M4 ON M3.UOM=M4.Code WHERE M3.Code=C.Paper4) As Paper4Name,[PaperWastage4%],PaperConsumptionOther4,(SELECT ''+LTrim(M6.PrintName)+' ' FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper4) As UOM4,(SELECT LTrim(M7.[Weight/Unit]) FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper4) As UOM4wt,(SELECT LTrim(M6.[Value1]) FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper4) As UOM4qty," & _
                                  "TotalPaperConsumption,Adjustment,PAdjustment,[VAT%],VAT,[PVAT%],PVAT,BillAmount,PBillAmount,C.Remarks,(SELECT LTRIM(eMail) FROM AccountMaster WHERE CODE=P.BookPrinter) As EMailId,M1.Narration,P.BookPrinter,PlateMaker,PaperWastageMin1,PaperWastageMin2,PaperWastageMin4,PaperRate1,PaperRate2,PaperRate4,PaperAmount1,PaperAmount2,PaperAmount4,RBillAmount,RAdjustment,[RVAT%],RVAT,C.Processing,P.EstQty01 As FinalQuantity,P.ProfitMargin,C.Ref,C.[TotalForms1-¼],C.[TotalForms1-½],C.[TotalForms1-1],C.[TotalPlates1-¼],C.[TotalPlates1-½],C.[TotalPlates1-1],C.[RevisedPlates1],C.[TotalForms2-¼],C.[TotalForms2-½],C.[TotalForms2-1],C.[TotalPlates2-¼],C.[TotalPlates2-½],C.[TotalPlates2-1],C.[RevisedPlates2],C.[TotalForms4-¼],C.[TotalForms4-½],C.[TotalForms4-1],C.[TotalPlates4-¼],C.[TotalPlates4-½],C.[TotalPlates4-1],C.[RevisedPlates4],C.PaperConsumptionsheets1,C.PaperConsumptionsheets2,C.PaperConsumptionsheets4,P.Type  " & _
                                  "FROM ((BookPOParent P INNER JOIN BookPOChild05 C ON P.Code=C.Code) INNER JOIN BookMaster M1 ON P.Book=M1.Code) INNER JOIN GeneralMaster M2 ON M1.FinishSize=M2.Code WHERE P.Code='" & OrderCode & "' AND P.BookPrinter='" & rstBookPOChild0801.Fields("Vendor").Value & "'", cnDatabase, adOpenKeyset, adLockOptimistic
    Else
                  rstBookPOChild05.Open "SELECT LTRIM(M1.PrintName)+iif(M1.Price=0,'',' (Price : Rs. '+Format(M1.Price,'0.00')+')') As Item,M2.PrintName As FinishSize,LTRIM(LTRIM(M1.Pages)+'-pages/'+LTRIM(M1.Forms)+'f('+IIF(M1.OneColorForms=0,'','1Col_'+LTRIM(M1.OneColorForms)+'f ')+IIF(M1.TwoColorForms=0,'',' 2Col_'+LTRIM(M1.TwoColorForms)+'f ')+IIF(M1.FourColorForms=0,'',' 4Col-'+LTRIM(M1.FourColorForms)+'f '))+')' As Forme,LTRIM(P.Name) As OrderNo,P.Date As OrderDate,(SELECT PrintName FROM AccountMaster WHERE Code=P.BookPrinter) As TextPrinter,C.ActualQuantity,M1.DuplexPrinting,BillingQuantity01,BillingQuantity02,(SELECT PrintName+'/'+CHOOSE(M1.FormType,'08','16','04','12','24','32','64','06') FROM GeneralMaster WHERE Code=C.Size1) As [Size1],Pages1,[Forms1-¼],[Forms1-½],[Forms1-1],CHOOSE(VAL(PlateType1),'Deep-etch','PS','Wipe-on','CTP') As Plate1,PrintRate1,PrintAmount1,PlateRate1,PlateAmount1," & _
                                  "(SELECT LTRIM(M3.PrintName) FROM PaperMaster M3 INNER JOIN GeneralMaster M4 ON M3.UOM=M4.Code WHERE M3.Code=C.Paper1) As Paper1Name,[PaperWastage1%],PaperConsumptionOther1,(SELECT ''+LTrim(M6.PrintName)+' ' FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper1) As UOM1,(SELECT LTrim(M7.[Weight/Unit]) FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper1) As UOM1wt,(SELECT LTrim(M6.[Value1]) FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper1) As UOM1qty," & _
                                  "(SELECT PrintName+'/'+CHOOSE(M1.FormType,'08','16','04','12','24','32','64','06') FROM GeneralMaster WHERE Code=C.Size2) As [Size2],Pages2,[Forms2-¼],[Forms2-½],[Forms2-1],CHOOSE(VAL(PlateType2),'Deep-etch','PS','Wipe-on','CTP') As Plate2,PrintRate2,PrintAmount2,PlateRate2,PlateAmount2," & _
                                  "(SELECT LTRIM(M3.PrintName) FROM PaperMaster M3 INNER JOIN GeneralMaster M4 ON M3.UOM=M4.Code WHERE M3.Code=C.Paper2) As Paper2Name,[PaperWastage2%],PaperConsumptionOther2,(SELECT ''+LTrim(M6.PrintName)+' ' FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper2) As UOM2,(SELECT LTrim(M7.[Weight/Unit]) FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper2) As UOM2wt,(SELECT LTrim(M6.[Value1]) FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper2) As UOM2qty," & _
                                  "(SELECT PrintName+'/'+CHOOSE(M1.FormType,'08','16','04','12','24','32','64','06') FROM GeneralMaster WHERE Code=C.Size4) As [Size4],Pages4,[Forms4-¼],[Forms4-½],[Forms4-1],CHOOSE(VAL(PlateType4),'Deep-etch','PS','Wipe-on','CTP') As Plate4,PrintRate4,PrintAmount4,PlateRate4,PlateAmount4," & _
                                  "(SELECT LTRIM(M3.PrintName) FROM PaperMaster M3 INNER JOIN GeneralMaster M4 ON M3.UOM=M4.Code WHERE M3.Code=C.Paper4) As Paper4Name,[PaperWastage4%],PaperConsumptionOther4,(SELECT ''+LTrim(M6.PrintName)+' ' FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper4) As UOM4,(SELECT LTrim(M7.[Weight/Unit]) FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper4) As UOM4wt,(SELECT LTrim(M6.[Value1]) FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper4) As UOM4qty," & _
                                  "TotalPaperConsumption,Adjustment,PAdjustment,[VAT%],VAT,[PVAT%],PVAT,BillAmount,PBillAmount,C.Remarks,(SELECT LTRIM(eMail) FROM AccountMaster WHERE CODE=P.BookPrinter) As EMailId,M1.Narration,P.BookPrinter,PlateMaker,PaperWastageMin1,PaperWastageMin2,PaperWastageMin4,PaperRate1,PaperRate2,PaperRate4,PaperAmount1,PaperAmount2,PaperAmount4,RBillAmount,RAdjustment,[RVAT%],RVAT,C.Processing,P.EstQty01 As FinalQuantity,P.ProfitMargin,C.Ref,C.[TotalForms1-¼],C.[TotalForms1-½],C.[TotalForms1-1],C.[TotalPlates1-¼],C.[TotalPlates1-½],C.[TotalPlates1-1],C.[RevisedPlates1],C.[TotalForms2-¼],C.[TotalForms2-½],C.[TotalForms2-1],C.[TotalPlates2-¼],C.[TotalPlates2-½],C.[TotalPlates2-1],C.[RevisedPlates2],C.[TotalForms4-¼],C.[TotalForms4-½],C.[TotalForms4-1],C.[TotalPlates4-¼],C.[TotalPlates4-½],C.[TotalPlates4-1],C.[RevisedPlates4],C.PaperConsumptionsheets1,C.PaperConsumptionsheets2,C.PaperConsumptionsheets4  " & _
                                  "FROM ((BookPOParent P INNER JOIN BookPOChild05 C ON P.Code=C.Code) INNER JOIN BookMaster M1 ON P.Book=M1.Code) INNER JOIN GeneralMaster M2 ON M1.FinishSize=M2.Code WHERE P.Code='" & OrderCode & "' AND P.BookPrinter='" & rstBookPOChild0801.Fields("Vendor").Value & "'", cnDatabase, adOpenKeyset, adLockOptimistic
    End If
            ...
            Else
                ...
                rptBookPrintOrder02.Text64.SetText rstBookPOChild05.Fields("Forme").Value
                ...
                If rstBookPOChild05.RecordCount <> 0 Then
                If Val(rstBookPOChild05.Fields("ProfitMargin").Value) = 0 And profitFlag = False Then
                rptBookPrintOrder02.Section17.Suppress = True
                ElseIf Val(rstBookPOChild05.Fields("ProfitMargin").Value) <> 0 And profitFlag = False Then
                profitFlag = True
                rptBookPrintOrder02.Section17.Suppress = False
        ...
        If OrderType = "TP" Or OrderType = "ALL" Then 'Title or All Printing
            ...
            If DatabaseType = "MS SQL" Then
                rstBookPOChild06.Open "SELECT (SELECT PrintName FROM ElementMaster WHERE Code=C.Element) As Element,C.Pages," & _
                                                                   "LTRIM(M1.PrintName)+iif(M1.Price=0,'',' (Price : Rs. '+Format(M1.Price,'0.00')+')') As Item,M2.PrintName As FinishSize,LTRIM(LTRIM(M1.Pages)+'-pages/'+LTRIM(M1.Forms)+'f('+IIF(M1.OneColorForms=0,'','1Col-'+LTRIM(M1.OneColorForms)+'f ')+IIF(M1.TwoColorForms=0,'',' 2Col-'+LTRIM(M1.TwoColorForms)+'f ')+IIF(M1.FourColorForms=0,'',' 4Col-'+LTRIM(M1.FourColorForms)+'F'))+')' As Forme,LTRIM(P.Name) As OrderNo,P.Date As OrderDate,(SELECT PrintName FROM AccountMaster WHERE Code=P.TitlePrinter) As CoverPrinter,C.ActualQuantity,C.BillingQuantity,C.FrontPrintingType,C.BackPrintingType,Imposition,(SELECT PrintName FROM GeneralMaster WHERE Code=C.[Size]) As [Size],CHOOSE(CONVERT(NUMERIC,PlateType),'Deep-etch','PS','Wipe-on','CTP') As Plate,CHOOSE(CONVERT(NUMERIC,PlateTypeBack),'Deep-etch','PS','Wipe-on','CTP') As PlateBack,C.TotalPlates,C.TotalPlatesBack," & _
            ...
            Else
                rstBookPOChild06.Open "SELECT (SELECT PrintName FROM ElementMaster WHERE Code=C.Element) As Element,C.Pages," & _
                                                                   "LTRIM(M1.PrintName)+iif(M1.Price=0,'',' (Price : Rs. '+Format(M1.Price,'0.00')+')') As Item,M2.PrintName As FinishSize,LTRIM(LTRIM(M1.Pages)+'-pages/'+LTRIM(M1.Forms)+'f('+IIF(M1.OneColorForms=0,'','1Col_'+LTRIM(M1.OneColorForms)+'f ')+IIF(M1.TwoColorForms=0,'',' 2Col_'+LTRIM(M1.TwoColorForms)+'f ')+IIF(M1.FourColorForms=0,'',' 4Col_'+LTRIM(M1.FourColorForms)+'F'))+')' As Forme,LTRIM(P.Name) As OrderNo,P.Date As OrderDate,(SELECT PrintName FROM AccountMaster WHERE Code=P.TitlePrinter) As CoverPrinter,C.ActualQuantity,C.BillingQuantity,C.FrontPrintingType,C.BackPrintingType,Imposition,(SELECT PrintName FROM GeneralMaster WHERE Code=C.[Size]) As [Size],CHOOSE(Val(PlateType),'Deep-etch','PS','Wipe-on','CTP') As Plate,CHOOSE(Val(PlateTypeBack),'Deep-etch','PS','Wipe-on','CTP') As PlateBack,C.TotalPlates,C.TotalPlatesBack," & _
                                                                   "PrintRate,PrintRateBack,PrintAmount,PlateRate,PlateRateBack,PlateAmount,(SELECT LTRIM(M3.PrintName) FROM PaperMaster M3 INNER JOIN GeneralMaster M4 ON M3.UOM=M4.Code WHERE M3.Code=C.Paper) As PaperName," & _
                                                                   "[PaperWastage%],[PaperWastage%Back],PaperWastageFinal,PaperConsumptionOther,PaperConsumptionKg,Adjustment,PAdjustment,[VAT%],VAT,[PVAT%],PVAT,BillAmount,PBillAmount,C.Remarks,(SELECT LTRIM(eMail) FROM AccountMaster WHERE Code=P.TitlePrinter) As EMailId,M1.Narration,P.TitlePrinter,PlateMaker,PaperWastageMin,PaperWastageMinBack,PaperRate,PaperAmount,RBillAmount,RAdjustment,[RVAT%],RVAT,C.Processing,C.ProcessingBack,C.[Titles/sheet1] As Ups,P.EstQty01 As FinalQuantity,P.ProfitMargin,C.Sets,(SELECT '('+LTRIM(M6.PrintName)+')' FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper) As UOM,C.Ref,P.Code As VchCode,C.PaperConsumptionsheets,(SELECT LTrim(M7.[Weight/Unit]) FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper) As UOMwt,(SELECT LTrim(M6.[Value1]) FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C.Paper) As UOMqty " & _
                                                                   "FROM ((BookPOParent P INNER JOIN BookPOChild06 C ON P.Code=C.Code) INNER JOIN BookMaster M1 ON P.Book=M1.Code) INNER JOIN GeneralMaster M2 ON C.FinishSize=M2.Code WHERE P.Code='" & OrderCode & "' AND P.TitlePrinter='" & rstBookPOChild0801.Fields("Vendor").Value & "'", cnDatabase, adOpenKeyset, adLockOptimistic
            End If
            If rstBookPOChild06.RecordCount = 0 Then
                rptBookPrintOrder02.Section4.Suppress = True
            Else
                ...
                If Not HeaderPrinted Then
                    ...
                    rptBookPrintOrder02.Text64.SetText rstBookPOChild06.Fields("Forme").Value
                    rptBookPrintOrder02.Text15.SetText rstBookPOChild06.Fields("FinalQuantity").Value
                    
                    If rstBookPOChild06.RecordCount <> 0 Then
                    If Val(rstBookPOChild06.Fields("ProfitMargin").Value) = 0 And profitFlag = False Then
                    rptBookPrintOrder02.Section17.Suppress = True
                    ElseIf Val(rstBookPOChild06.Fields("ProfitMargin").Value) <> 0 And profitFlag = False Then
                    profitFlag = True
                    rptBookPrintOrder02.Section17.Suppress = False
                    rptBookPrintOrder02.Text44.SetText Format(rstBookPOChild06.Fields("ProfitMargin").Value / 100, "##0.00 %")
                    End If
                    End If
        ...
        Else
                rstBookPOChild10.Open "SELECT LTRIM(P.Name) As OrderNo,P.Date As OrderDate,TargetDate,Plate,(SELECT PrintName FROM AccountMaster WHERE Code=P.TitlePrinter) As CoverPrinter,(SELECT PrintName FROM AccountMaster WHERE Code=C1.PlateMaker) As PlateMakers,CHOOSE(Val(C1.PlateType),'Deep-etch','PS','Wipe-on','CTP') As PlateName,C1.TotalFormsFront,C1.TotalFormsBack,C1.PrintRateFront,C1.PrintRateBack,C1.[GST%],C1.GST,C1.Adjustment,C1.PrintAmount,C1.TotalPlates,C1.PlateRate,C1.PAdjustment,C1.[PGST%],C1.PGST,PlateAmount," & _
                          "(SELECT LTRIM(M3.PrintName)FROM PaperMaster M3 INNER JOIN GeneralMaster M4 ON M3.UOM=M4.Code WHERE M3.Code=C1.Paper) As PaperName,C1.[PaperWastage%],C1.[PaperConsumptionOther],C1.Remarks,(SELECT PrintName FROM GeneralMaster WHERE Code=C1.[Size]) As [Size],LTRIM(M1.PrintName)+iif(M1.Price=0,'',' (Price : Rs. '+Format(M1.Price,'0.00')+')') As Item,C2.ActualQuantity,[Ups/Plate],C2.PrintingQuantity,C2.BillingQuantity,LTRIM(C2.FrontPrintingColor)+'+'+LTRIM(C2.BackPrintingColor) + ' ('+IIF(C1.Imposition='F','F&B','W&T')+')' As PrintingType,P.TitlePrinter AS TitlePrinterCode,C1.PlateMaker As PlateMakerCode," & _
                          "PaperWastageMin,PaperRate,PaperAmountBT,RAdjustment,[RGST%],RGST,PaperAmount,PrintAmountBT,PlateAmountBT,(SELECT EMail FROM AccountMaster WHERE Code=P.TitlePrinter) As EMail,Imposition,PlateType,(SELECT '('+LTRIM(M6.PrintName)+')' FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C1.Paper) As UOM,(SELECT LTRIM(MAX(PrintingQuantity)) FROM BookPOChild0901 WHERE Code=P.Code) As MaxPrintingQty, " & _
                          "P.EstQty01 As FinalQuantity,P.ProfitMargin,(SELECT LTRIM(MAX(FrontPrintingColor)) FROM BookPOChild0901 WHERE Code=P.Code) As MaxFrontColor,(SELECT LTRIM(MAX(BackPrintingColor)) FROM BookPOChild0901 WHERE Code=P.Code) As MaxBackColor,C1.Calculation,C1.PaperConsumptionsheets,(SELECT LTrim(M7.[Weight/Unit]) FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C1.Paper) As UOMwt,(SELECT LTrim(M6.[Value1]) FROM PaperMaster M7 INNER JOIN GeneralMaster M6 ON M7.UOM=M6.Code WHERE M7.Code=C1.Paper) As UOMqty FROM ((BookPOParent P INNER JOIN BookPOChild09 C1 ON P.Code=C1.Code) INNER JOIN BookPOChild0901 C2 ON C1.Code=C2.Code) INNER JOIN BookMaster M1 ON C2.Book=M1.Code WHERE P.Code='" & OrderCode & "'", cnDatabase, adOpenKeyset, adLockOptimistic
...
'                    rptBookPrintOrder02.Text64.SetText rstBookPOChild10.Fields("Forme").Value
                ...
                If rstBookPOChild10.RecordCount <> 0 Then
                If Val(rstBookPOChild10.Fields("ProfitMargin").Value) = 0 And profitFlag = False Then
                rptBookPrintOrder02.Section17.Suppress = True
                ElseIf Val(rstBookPOChild10.Fields("ProfitMargin").Value) <> 0 And profitFlag = False Then
                profitFlag = True
                rptBookPrintOrder02.Section17.Suppress = False
                rptBookPrintOrder02.Text44.SetText Format(rstBookPOChild10.Fields("ProfitMargin").Value / 100, "##0.00 %")
                End If
                End If
        ...
        If OrderType = "TL" Or OrderType = "ALL" Then
            ...
            Else
                ...
                If rstBookPOChild07.RecordCount <> 0 Then
                If Val(rstBookPOChild07.Fields("ProfitMargin").Value) = 0 And profitFlag = False Then
                rptBookPrintOrder02.Section17.Suppress = True
                ElseIf Val(rstBookPOChild07.Fields("ProfitMargin").Value) <> 0 And profitFlag = False Then
                profitFlag = True
                rptBookPrintOrder02.Section17.Suppress = False
                rptBookPrintOrder02.Text44.SetText Format(rstBookPOChild07.Fields("ProfitMargin").Value / 100, "##0.00 %")
                End If
                End If
        ...
        If OrderType = "BB" Or OrderType = "ALL" Then
            ...
            Else
                ...
                If Not HeaderPrinted Then
                    ...
                    If rstBookPOChild08.RecordCount <> 0 Then
                    If Val(rstBookPOChild08.Fields("ProfitMargin").Value) = 0 And profitFlag = False Then
                    rptBookPrintOrder02.Section17.Suppress = True
                    ElseIf Val(rstBookPOChild08.Fields("ProfitMargin").Value) <> 0 And profitFlag = False Then
                    profitFlag = True
                    rptBookPrintOrder02.Section17.Suppress = False
                    rptBookPrintOrder02.Text44.SetText Format(rstBookPOChild08.Fields("ProfitMargin").Value / 100, "##0.00 %")
                    End If
                    End If
                    EMailID = rstBookPOChild08.Fields("EMailId").Value
                    OrderNo = rstBookPOChild08.Fields("OrderNo").Value
...
'            rptBookPrintOrder02.Section14.Suppress = True
        ...
        End If
        If rstBookPOChild09.RecordCount <> 0 Then
        If Val(rstBookPOChild09.Fields("ProfitMargin").Value) = 0 And profitFlag = False Then
        rptBookPrintOrder02.Section17.Suppress = True
        ElseIf Val(rstBookPOChild09.Fields("ProfitMargin").Value) <> 0 And profitFlag = False Then
        profitFlag = True
        rptBookPrintOrder02.Section17.Suppress = False
        rptBookPrintOrder02.Text44.SetText Format(rstBookPOChild09.Fields("ProfitMargin").Value / 100, "##0.00 %")
        End If
        End If
        If TotalTax = 0 Then rptBookPrintOrder02.Text39.SetText " ": rptBookPrintOrder02.Text40.SetText " ": If TotalTax = 0 Then rptBookPrintOrder02.Text33.SetText " ": rptBookPrintOrder02.Text30.SetText " "
...
Private Function Save2Master() As Boolean
    ...
    cnBookPrintOrder.Execute "INSERT INTO BookChild06 SELECT TOP 1 Book As Code,Element,Pages,[FinishSize],[Size],Imposition,FrontPrintingType,BackPrintingType,PlateType,PlateTypeBack,[Titles/sheet1],P.Type FROM BookPOParent P INNER JOIN BookPOChild06 C ON P.Code=C.Code WHERE P.Type='" & BookPOType & "' AND P.Book='" & ItemCode & "' ORDER BY P.Code DESC"
    Exit Function
ErrHandler:
    Save2Master = False
End Function
Private Sub MhRealInput8_Validate(Cancel As Boolean)
    ...
    With rstBookPOChild08
        If .RecordCount <> 0 Then
            .MoveFirst
            Do While Not .EOF
                MhRealInput28.Value = MhRealInput28.Value + (Val(.Fields("FormFoldRate").Value) * Val(.Fields("ActualQuantity").Value) * (Val(.Fields("BindingForms").Value) + Val(.Fields("ExtraForms").Value))) / 1000
                MhRealInput28.Value = MhRealInput28.Value + (Val(.Fields("FormPasteRate").Value) * Val(.Fields("ActualQuantity").Value)) / 1000
                MhRealInput28.Value = MhRealInput28.Value + (Val(.Fields("FormStitchRate").Value) * Val(.Fields("ActualQuantity").Value) * (Val(.Fields("BindingForms").Value) + Val(.Fields("ExtraForms").Value))) / 1000
                MhRealInput28.Value = MhRealInput28.Value + Val(.Fields("Rate/Book").Value) * Val(.Fields("ActualQuantity").Value)
                MhRealInput28.Value = MhRealInput28.Value + Val(.Fields("TotalPkts").Value) * Val(.Fields("PktPackRate").Value)
                MhRealInput28.Value = MhRealInput28.Value + Val(.Fields("TotalBoxes").Value) * Val(.Fields("BoxPackRate").Value)
                MhRealInput28.Value = MhRealInput28.Value + Val(.Fields("TotalBoxes").Value) * Val(.Fields("CartageRate").Value)
                MhRealInput28.Value = MhRealInput28.Value + Val(.Fields("Adjustment").Value)
                .MoveNext
            Loop
    ...
    With rstBookPOChild09
        If .RecordCount <> 0 Then
            .MoveFirst
            Do While Not .EOF
                Amount = Val(.Fields("PlateAmountBT").Value) + Val(.Fields("PAdjustment").Value) 'Plate Amount
                PlateAmount = PlateAmount + Amount
                MhRealInput24.Value = MhRealInput24.Value + Amount
                Amount = Val(.Fields("PaperAmountBT").Value) + Val(.Fields("RAdjustment").Value) 'Paper Amount
                OthersAmount = OthersAmount + Amount
                MhRealInput24.Value = MhRealInput24.Value + Amount
                Amount = Val(.Fields("PrintAmountBT").Value) + Val(.Fields("Adjustment").Value) 'Print Amount
                OthersAmount = OthersAmount + Amount
                MhRealInput24.Value = MhRealInput24.Value + Amount
                .MoveNext
            Loop
