...
Private Sub GetBinderRates()
    ...
    If rstBinderRates.RecordCount = 0 Then
        If rstBinderRates.State = adStateOpen Then rstBinderRates.Close
        rstBinderRates.Open "Select Top 1 * From AccountChild08 Where Code = '" & BinderCode & "' And Left((Select Name From GeneralMaster where code=[Size]),5)*Convert(numeric,Substring((Select Name From GeneralMaster where code=[Size]),7,5))>=Left((Select Name From GeneralMaster WHERE Code='" & SizeCode & "'),5)*Convert(numeric,Substring((Select Name From GeneralMaster WHERE Code='" & SizeCode & "'),7,5)) And BindingType = '" & BindingTypeCode & "' And Range" & Choose(Val(FormType), "08", "16", "04", "12", "24", "32", "64", "06", "02") & " >= " & Val(MhRealInput7.Text) + Val(MhRealInput18.Text) & " Order By Range" & IIf(FormType = "1", "08", IIf(FormType = "2", "16", IIf(FormType = "3", "04", IIf(FormType = "4", "12", IIf(FormType = "5", "24", IIf(FormType = "6", "32", "64")))))), cnDatabase, adOpenKeyset, adLockReadOnly
    End If
    If rstBinderRates.RecordCount = 0 Then
        If rstBinderRates.State = adStateOpen Then rstBinderRates.Close
